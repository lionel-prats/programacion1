        ...
        for row in self.screen_configs.get("levels").get(str(current_level)): 
            ...
            for tile in row: 
                ...
                if tile == 1: # dirt
                    ...
                    enemy = Blob(blob_path_image, coord_x, coord_y, self.tile_size, 1, 42, self.tile_list, enemy_sprite_group)
                    ...
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 1, 0, 1, 40)
                    platform_group.add(platform)

                elif tile == 5: # plataformas Y UP 40 
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 0, 1, -1, 40)
                    platform_group.add(platform)
                ...
                elif tile == 9: # plataformas X LEFT 40
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 1, 0, -1, 40)
                    platform_group.add(platform)

                elif tile == 11: # plataformas Y DOWN 40
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 0, 1, 1, 40)
                    platform_group.add(platform)
                    ...
                    blob_path_image = enemy_configs.get("blob").get("path_image")
                    enemy = Blob(blob_path_image, coord_x, coord_y, self.tile_size, 1, 80, self.tile_list, enemy_sprite_group)
                    enemy_sprite_group.add(enemy)
                
                elif tile == 13: # blob RIGHT 120
                    blob_path_image = enemy_configs.get("blob").get("path_image")
                    enemy = Blob(blob_path_image, coord_x, coord_y, self.tile_size, 1, 120, self.tile_list, enemy_sprite_group)
                    enemy_sprite_group.add(enemy)
                
                elif tile == 14: # blob LEFT 40
                    blob_path_image = enemy_configs.get("blob").get("path_image")
                    enemy = Blob(blob_path_image, coord_x, coord_y, self.tile_size, -1, 40, self.tile_list, enemy_sprite_group)
                    enemy_sprite_group.add(enemy)
                    ...
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 1, 0, 1, 80)
                    platform_group.add(platform)

                elif tile == 18: # plataformas X LEFT 80
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 1, 0, -1, 80)
                    platform_group.add(platform)

                elif tile == 19: # plataformas Y DOWN 80 
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 0, 1, 1, 80)
                    platform_group.add(platform)

                elif tile == 20: # plataformas Y UP 80
                    platform_path_image = self.screen_configs.get("images").get("platform") 
                    platform = Platform(platform_path_image, coord_x, coord_y, self.tile_size, 0, 1, -1, 80)
                    platform_group.add(platform)
