...
participaciones en el All-Star y pertenencia al Salón de la Fama del Baloncesto, etc.\n""")
        ...
        print("8. Mostrar la lista de jugadores, ordenados de mayor a menor, según la suma de bloqueos totales + robos totales.\n")
        print("9. Salir\n")

        opcion = input("Seleccione una opción entre 1 y 9: ")
        ...
        match opcion:
            ...
            case "8":
                ...
                print(f"\n{opcion}) Mostrar la lista de jugadores, ordenados de mayor a menor, según la suma de bloqueos totales + robos totales.")

                input_usuario = input(f"\nIngrese la cantidad de jugadores a mostrar (cantidad entre 1 y {len(lista_jugadores)}): ")

                while not validar_dato(regex, input_usuario) or int(input_usuario) < 1 or int(input_usuario) > 12:
                    input_usuario = input(f"\nIngrese la cantidad de jugadores a mostrar (cantidad entre 1 y {len(lista_jugadores)}): ")
                
                print("")

                print_pantalla = ""
                for i in range(0, int(input_usuario)):
                    if i + 1 < 10:
                        print_pantalla += f"{i+1}  - "
                    else:
                        print_pantalla += f"{i+1} - "
                    for k, v in lista_jugadores[i].items(): 
                        if k == "nombre":
                            print_pantalla += f"{v} | "
                        elif k == "porcentaje":
                            print_pantalla += f"{k.replace('_', ' ').capitalize()}: {v}%"
                        else:
                            print_pantalla += f"{k.replace('_', ' ').capitalize()}: {v} | "
                    print_pantalla += "\n"

                print_pantalla = print_pantalla[:-1]
                print(print_pantalla)
            ...
            case _:
                limpiar_consola()
                print("\nOpción no válida. Debe seleccionar una opción entre 1 y 9.")
                separador()