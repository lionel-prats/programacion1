...
while jugando:
    ...
    reloj.tick(60) # milisegundos de una iteracion del bucle
    ...
    milisegundos = fuente.render(f"MILISEGUNDOS TRANSCURRIDOS = {str(pygame.time.get_ticks())}", True, AZUL)
    segundos = fuente.render(f"SEGUNDOS TRANSCURRIDOS = {str(pygame.time.get_ticks() / 1000)}", True, AZUL)
    fps = fuente.render(f"FPS = {str(reloj.get_fps())}", True, AZUL)
    ventana.blit(milisegundos, ((ANCHO - milisegundos.get_width())/2,200))
    ventana.blit(segundos, ((ANCHO - segundos.get_width())/2,300))
    ventana.blit(fps, ((ANCHO - segundos.get_width())/2,400))
